...
import { validateAndOptimizeContent, getOptimizedBlueprintResult, ValidationResult } from '@/lib/contentValidator';
...
const AiMessageBubble: React.FC<AiMessageBubbleProps> = ({
  ...
  const handleGenerateBlueprint = async (isAutoGenerated = false) => {
    ...
    try {
      ...
      // Auto-validate content after blueprint is generated (only for auto-generated blueprints)
      if (isAutoGenerated && !hasAutoValidated) {
        console.log('Auto-validating content after blueprint completion...');
        setHasAutoValidated(true);
        await handleValidateContent(generatedBlueprint, true);
      }
    } catch (error) {
...
#干货分享 #学习笔记 #实用技巧 #知识分享 #效率提升 #生活技能 #经验总结 #必看推荐 #涨知识`
  ...
  const handleValidateContent = async (providedBlueprint?: BlueprintResult, isAutoGenerated = false) => {
    console.log(isAutoGenerated ? 'Auto-validating content...' : 'Content validation triggered...');
    try {
      const blueprintToUse = providedBlueprint || generatedBlueprint;
      
      if (!blueprintToUse) {
        console.log('No blueprint available for validation');
        return;
      }
      
      console.log('Validating and optimizing content...');
      const validationResult = await validateAndOptimizeContent(blueprintToUse);
      console.log('Content validation completed:', validationResult);
      setValidationResult(validationResult);
      setShowValidation(true);
      
      // For auto-generated validation, continue to HTML generation automatically
      if (isAutoGenerated && !hasAutoGeneratedHTML) {
        console.log('Auto-generating HTML after validation completion...');
        setHasAutoGeneratedHTML(true);
        const optimizedBlueprint = getOptimizedBlueprintResult(validationResult);
        await handleGenerateHTML(optimizedBlueprint, true);
      }
    } catch (error) {
      console.error('Error validating content:', error);
      // If validation fails, continue with original blueprint
      if (isAutoGenerated && !hasAutoGeneratedHTML && providedBlueprint) {
        console.log('Validation failed, continuing with original blueprint...');
        setHasAutoGeneratedHTML(true);
        await handleGenerateHTML(providedBlueprint, true);
      }
    }
  };

  const handleValidationContinue = async () => {
    console.log('User confirmed validation, proceeding to HTML generation...');
    if (validationResult) {
      const optimizedBlueprint = getOptimizedBlueprintResult(validationResult);
      await handleGenerateHTML(optimizedBlueprint, false);
    }
  };
  ...
  const handleHTMLButtonClick = () => {
    // Use validated blueprint if available, otherwise use original blueprint
    const blueprintToUse = validationResult 
      ? getOptimizedBlueprintResult(validationResult) 
      : generatedBlueprint || undefined;
    handleGenerateHTML(blueprintToUse);
  };
  ...
  const handleGeneratePNG = async (providedHTML?: string, providedBlueprint?: BlueprintResult) => {
    ...
    // Prioritize optimized blueprint from validation, then provided blueprint, then original blueprint
    let blueprintToUse = providedBlueprint || generatedBlueprint;
    if (validationResult && !providedBlueprint) {
      blueprintToUse = getOptimizedBlueprintResult(validationResult);
    }
  ...
  return (
    <div className={`relative break-words flex flex-col`}>
      ...
      {showValidation && validationResult && (
        <div className="mt-4 p-6 bg-orange-900/20 rounded-lg border-2 border-orange-500/30 shadow-lg">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button
              ...
              className="text-orange-400 hover:text-orange-200 hover:bg-orange-800/30"
            >
              <X className="h-4 w-4" />
            </Button>
          </div>
      ...
      <div className="flex gap-2 mt-4 mb-2">
        ...
        <Button
          variant="outline"
          size="sm"
          className="bg-orange-700 hover:bg-orange-600 text-orange-100 border-orange-500"
          onClick={() => generatedBlueprint ? handleValidateContent(generatedBlueprint, false) : null}
          disabled={!generatedBlueprint}
        >
